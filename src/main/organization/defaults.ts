...
export function defaultPolicies(companyId: string): OrganizationPolicy[] {
  const rows: Array<[string, OrganizationPolicy['effect'], string]> = [
    ...
    ['source.read', 'allow', 'Bounded workspace reads and searches are allowed for internal task execution.'],
    ['git.status', 'allow', 'Read-only Git status inspection is allowed for internal task execution.'],
    ...
    ['file.download', 'ask', 'Browser downloads require governed approval by default.'],
    ['file.upload', 'ask', 'Uploading local files through a browser requires governed approval by default.'],
    ['workflow.complete', 'allow', 'A task may report completion only through the normal machine-verification gate.'],
    ['source.write', 'ask', 'Source mutations stay on the governed execution path.'],
    ['shell.execute', 'ask', 'Shell execution stays on the governed execution path.'],
    ['git.commit', 'ask', 'Git commits stay on the governed execution path.'],
    ['git.push', 'ask', 'Remote Git mutation requires governed approval.'],
    ['runtime.retry', 'ask', 'Retries that change execution state remain governed.'],
    ['runtime.escalation', 'ask', 'Escalating to the full model tier remains governed.'],
