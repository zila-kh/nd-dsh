...
5. **Third-party license notices** — only `LICENSE.nd-dsh` ships today (`electron-builder.yml` `extraResources`). Produce a notices bundle covering the redistributed surface (Harness closure, agent-browser, ND Pencil upstream notices, npm runtime deps) and add a `release:verify` step that fails when a redistributed root lacks a notice. **Accept:** notices file present in the artifact; staging closure and notices agree.
...
7. **Docs/QA hygiene** — `docs/qa/` still reflects the 2026-09-05 beta-v1 state; refresh one current handoff or mark the folder as superseded by the dated evidence in task records. (README status, the task board 0020–0030 archive, and this document were synced in this pass.)
