...
- [x] The local gates pass. — `pnpm verify`, `pnpm typecheck`, `pnpm bench:tasks:check` and `pnpm test` (99 files passed, 4 skipped) all exit 0. One gate failure was repaired rather than explained away (see §5), and one unrelated flake found on the way is filed as [todo-0017](../todo-0017-windows-worktree-test-ebusy-flake.md).
