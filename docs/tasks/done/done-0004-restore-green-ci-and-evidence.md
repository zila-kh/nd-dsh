...
> Priority: P0  
> Owner: ZCode  
> Branch: `chore/finalize-prd-0003-evidence` (archive/final evidence); implementation merged via PR #23  
> Updated: 2026-09-22  
...
| 3 — `validate` / Desktop smoke | **Failure named and classified: no spec failed.** Both non-draft runs ended in Playwright's worker teardown watchdog after 36 passed / 1 skipped. The Windows-only product defect found while reproducing it is fixed here; the teardown itself is [done-0010](done-0010-desktop-smoke-teardown.md). |
...
- [x] `performance-evidence` completes and uploads a `benchmark-results/**` artifact. — **Final archive merge gate:** this is proven by the finalization PR's ready-for-review run; if the job does not complete successfully with its artifact, this archive must not merge.
