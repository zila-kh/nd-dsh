...
| Optional performance-evidence bundle | Attempted once (run `35771982331`) and failed on [done-0014](done/done-0014-app-runtime-terminal-marker.md), now fixed and reproduced before/after locally |
...
The implementation side of this record is complete. The three defects that the failed runs exposed are closed: [done-0012](done/done-0012-nd-core-format-lint-gate.md) restored `Verify ND Core` (a formatting/lint repair only — `cargo test -p nd-core` already passed 55 tests), [done-0013](done/done-0013-windows-timing-flakes.md) removed the Windows timing assumptions the repaired gate then revealed, and [done-0014](done/done-0014-app-runtime-terminal-marker.md) fixed the app-runtime terminal marker that failed the first `performance-evidence` attempt. All three merged with PR #30 (`8fd7c16`) and are verified locally. What remains is exactly the runner: this record's exit criteria are testable only on a Windows runner, and Actions is parked, so they wait for the workflows to be restored rather than for more code. A further Windows-only failure at that point would be a new defect and should be filed as its own task.
